...
<div>Nom du plat</div><div>Description</div><div>Prix</div><div>Visible</div><div></div><div></div>
</div>
...
addItem(catId, { name, description, price }, form);
});
});
}
...
method: "PUT",
body: JSON.stringify({ name, description, price, available: currentAvailable }),
});
